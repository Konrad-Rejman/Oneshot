...
The fine-tuned weights are deliberately **not distributed** — the public repository carries the pipeline and documentation only. With the synthetic-only dataset (see 3.1) the attribution chain collapsed to a single entry, already written as `training/NOTICE`:
